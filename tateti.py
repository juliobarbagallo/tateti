...
def insert_letter(letter, pos):
    """Insert letter (X or O) in given position."""
    board[pos] = letter


def pos_is_free(pos):
    """Boolean function, return True if the position on the board is available."""
    return board[pos] == ' '


def print_board(board):
    """It just make the prints to reflect a ta-te-ti board."""
    print('   |   |')
...
def is_winner(bo, le):
    """It gets a board and a letter and checks if horizontal lines (7,8 and 9) or
...
def player_move():
    """It tries to set the letter X (player) to desired board position."""
    run = True
...
def machine_move():
    """It tries the machine move. As its request, this point is deeper explained at the README file."""
...
def pick_random(li):
    """From a list of available positions, it randomly pick
    one of them to make the machine move."""
    #import random
    ln = len(li)
...
def is_board_full(board):
    """It use count function in order to count how many blanks (' ')
    are in the board and return True if there aren't blank spaces
    or False if there is any."""
    if board.count(' ') > 1:
        return False
    return True
...
def main():
    ...
    random init between user and machine"""
